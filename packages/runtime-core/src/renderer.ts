...
>(options: RendererOptions<HostNode, HostElement>) {
  // xiimao 3.createRenderer调用 baseCreateRenderer方法
...
function baseCreateRenderer(
  ...
  const patch: PatchFn = (
    ...
    /**
     * xiimao vnode(n2) vue3.0把vnode 的节点通过shapeFlag区分
     * 以当前 App 根节点为例 当前shapeFlag为4 === ShapeFlags.STATEFUL_COMPONENT(有状态组件)  对应的二进制 0100
     */
    ...
    switch (type) {
      ...
      case Fragment:
        ...
        break
      default:
        // xiimao ShapeFlags.ELEMENT === 0b0001
        // xiimao 22.如果是元素节点 则开始处理元素节点
        if (shapeFlag & ShapeFlags.ELEMENT) {
          ...
          // xiimao ShapeFlags.ELEMENT === 0b0110
  ...
  const mountElement = (
    ...
    if (
      ...
      // xiimao 跨平台方法。再浏览器是 document.createElement
      // xiimao 同时拿到当前节点
      ...
      if (shapeFlag & ShapeFlags.TEXT_CHILDREN) {
        ...
        // xiimao 23.如果是children节点，则mount子节点 其中容器变成了当前节点，即这样不断递归下去
        mountChildren(
  ...
  const processFragment = (
    ...
    if (n1 == null) {
      // xiimao 21.n1为null 表示初始化节点，目前是到了App的子树vnode，是一个fragment节点
      hostInsert(fragmentStartAnchor, container, anchor)
      hostInsert(fragmentEndAnchor, container, anchor)
      // a fragment can only have array children
      // since they are either generated by the compiler, or implicitly created
      // from arrays.
      // xiimao 初始化组件，一个一个往下塞
      mountChildren(
  ...
  const processComponent = (
    ...
    // xiimao 如果没有旧节点则代表创建组件，初始化的时候没有旧节点
    if (n1 == null) {
  ...
  /**
   * xiimao initialVNode 指的是从上图 processComponent 传入的n2，即当前需要初始化的vnode
   * xiimao 17.调用mountComponent
   */
  //
  ...
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    instance.update = effect(function componentEffect() {
      if (!instance.isMounted) {
        ...
        /**
         * xiimao 关于subTree， 可以理解为 vnode代表当前组件App，而subTree表示组件的内容是一个Fragment
         */
  ...
  // xiimao 4.baseCreateRenderer返回render和createApp方法
  return {
    render,
    hydrate,
